...
def run_auction(candidates: pd.DataFrame, *, quality_column: str, bid_column: str, mechanism: str, budget: float | None = None, pacing_multipliers: np.ndarray | None = None) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for order, (auction_id, group) in enumerate(candidates.groupby("synthetic_auction_id", sort=True)):
        ...
        multiplier = float(pacing_multipliers[order]) if pacing_multipliers is not None else 1.0
        ...
        curve.append({"auction_order": order, "synthetic_auction_id": int(auction_id), "cumulative_spend": spend, "payment": market_price if can_win else 0.0, "pacing_multiplier": multiplier})
